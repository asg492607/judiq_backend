...
from session import DatabaseManager, DB_PATH
...
class CaseroomManager:
    ...
    def get_full_caseroom_state(caseroom_id):
        ...
        # ðŸ”¥ INTELLIGENCE SYNC: Attach latest analysis to room state
        if state and state.get("room_info"):
            ...
            if case_record:
                import json
                try:
                    state["latest_analysis"] = json.loads(case_record[4]) # analysis_result is index 4
                except:
    ...
    def reanalyze_case_from_documents(caseroom_id, user_id="SYSTEM"):
        ...
        try:
            case_data = json.loads(original_case[3])  # case_data is index 3
        except:
