...
class PredictiveEngine:
    ...
    def calculate_settlement_probability(score: float, case_data: Dict[str, Any], concepts: list) -> Dict[str, Any]:
        """
        Suggests whether settlement is more likely than trial.
        """
        base_prob = 50.0
        factors = []
    ...
    def forecast_penalty_and_compensation(score: float, case_data: Dict[str, Any], concepts: list) -> Dict[str, Any]:
        """
        Estimates interim compensation (Sec 143A) and likely sentencing ranges.
        """
        try:
            amt = float(case_data.get("amount", 0))
        except:
        ...
        # Section 143A Interim Compensation (typically 10-20% granted if case strong)
        ...
        # Final Compensation (Sec 357 CrPC)
        if amt > 0:
