...
def test_zero_score_gating(monkeypatch):
    ...
    assert result.get("draft") == "DRAFT GENERATION BLOCKED: Case score too low or fatal defects present."
...
def test_notice_delivery_contradiction(monkeypatch):
    ...
    assert any("Notice" in r for r in result.get("reasoning_trace", []))
